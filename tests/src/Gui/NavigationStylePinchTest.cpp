...
SoGesturePinchEvent updateEvent(double deltaZoom, double deltaAngle)
{
...
TEST(NavigationStylePinchAction, gestureStartMovesNothing)
{
    SoGesturePinchEvent event;
    event.state = SoGestureEvent::SbGSStart;
    event.deltaZoom = 2.0;
...
TEST(NavigationStylePinchAction, gestureEndMovesNothing)
{
    SoGesturePinchEvent event;
    event.state = SoGestureEvent::SbGSEnd;
    event.deltaZoom = 2.0;
    event.deltaAngle = 1.0;
...
TEST(NavigationStylePinchAction, anUnchangedZoomFactorIsANoOp)
{
    const auto event = updateEvent(1.0, 0.0);

    const auto action = Gui::NavigationStyle::pinchAction(&event, TouchTiltAllowed);

    EXPECT_FLOAT_EQ(action.zoomLogFactor, 0.0F);
}

TEST(NavigationStylePinchAction, rotationIsPassedThroughUnchanged)
{
    const auto event = updateEvent(0.0, -0.25);

    const auto action = Gui::NavigationStyle::pinchAction(&event, TouchTiltAllowed);

    ASSERT_TRUE(action.rotate);
    EXPECT_FLOAT_EQ(action.rotateAngle, -0.25F);
}

TEST(NavigationStylePinchAction, disablingTiltStopsRotationButKeepsZoom)
{
...
TEST(NavigationStylePinchAction, disablingTiltStopsARotateOnlyPinchEntirely)
{
    const auto event = updateEvent(0.0, 0.5);
...
TEST(NavigationStylePinchAction, allowingTiltRotatesByTheReportedAngle)
{
    const auto event = updateEvent(2.0, 0.5);
